...
class Widget_ContusFeaturedVideos_init extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		$div       = '<div id="featured-videos"  class="sidebar-wrap "><h3 class="widget-title"><a href="' . $site_url . '/?page_id=' . $moreName . '&amp;more=fea">' . $title . '</a></h3>';
		$show      = $instance['show'];
		...
		$moreF  = $wpdb->get_results(
					'SELECT COUNT( a.vid ) AS contus FROM ' . $wpdb->prefix . 'hdflvvideoshare a
					INNER JOIN ' . $wpdb->prefix . 'hdflvvideoshare_med2play b ON a.vid=b.media_id
					INNER JOIN ' . $wpdb->prefix . 'hdflvvideoshare_playlist p ON p.pid=b.playlist_id
					WHERE a.publish="1" AND p.is_publish="1" AND a.featured="1"'
				);
