...
if ( class_exists( 'ContusVideo' ) != true ) {											## checks the ContusVideo class has been defined if starts
	...
	class ContusVideo {																	## ContusVideo class starts
		...
		public function get_more_pageid() {												## function for getting more page ID starts
		...
		public function get_video_count() {												## function for getting video count starts
			$video_count = $this->_wpdb->get_var( 'SELECT count( * ) FROM ' . $this->_videoinfotable . ' WHERE featured=1 and publish=1' );
			return $video_count;
		}

		public function get_tag_name( $vid ) {											## function for getting Tag name starts
		...
		public function get_video_detail( $vid ) {										## function for getting video detail starts
			global $wpdb;
		...
		public function video_pid_detail( $pid, $type ) {									## function for getting playlist detail starts
			...
			$themediafiles = $wpdb->get_results( $select );
			return $themediafiles;
		}																			## function for getting playlist detail ends

		public function get_singlevideodata() {										## function for getting single video detail starts
			$query = 'SELECT * FROM ' . $this->_videoinfotable . ' WHERE featured=1 and publish=1 ORDER BY vid DESC LIMIT 0,1';
			return $this->_wpdb->get_row( $query );
		}

		public function get_featuredvideodata() {									## function for getting featured videos detail starts
		...
		public function get_featuredvideodata_banner() {							## function for getting featured videos for banner starts
			$query = 'SELECT distinct w.*,s.guid FROM ' . $this->_videoinfotable . ' w
					INNER JOIN ' . $this->_wpdb->prefix . 'hdflvvideoshare_med2play m ON m.media_id = w.vid
					INNER JOIN ' . $this->_wpdb->prefix . 'hdflvvideoshare_playlist p ON p.pid=m.playlist_id
					INNER JOIN ' . $this->_wpdb->prefix . 'posts s ON s.ID=w.slug
		...
		public function get_home_catthumbdata( $thumImageorder, $dataLimit ) {		## function for getting thumb details for home page category section starts
			global $wpdb;
		...
		public function get_thumdata( $thumImageorder, $where, $dataLimit ) {		## function for getting thumb details for home page starts
		...
		public function get_countof_thumdata( $thumImageorder, $where ) {			## function for getting count of videos in home page starts
